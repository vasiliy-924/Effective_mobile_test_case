...
func NewSubscription(repo repository.SubscriptionRepository, logger *slog.Logger) *Subscription {
...
func (s *Subscription) Create(ctx context.Context, in domain.CreateSubscription) (domain.Subscription, error) {
	if err := validateCreatePayload(in); err != nil {
...
func (s *Subscription) Cost(ctx context.Context, in CostInput) (domain.CostReport, error) {
	...
	if err != nil {
		return domain.CostReport{}, fmt.Errorf("%w: from: %v", apperrors.ErrInvalidArgument, err)
	}
	toT, err := month.Parse(in.To)
	if err != nil {
		return domain.CostReport{}, fmt.Errorf("%w: to: %v", apperrors.ErrInvalidArgument, err)
	}
